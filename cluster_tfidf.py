...
import math
from dataloader_tfidf import PatientDataset
...
def fit(epoch,test_dataset,model,center_embedding,label_token,text_recon_loss,y_bce_loss,cluster_loss,dataloader,optimizer,flag='train'):
    ...
    for i,c in enumerate(visit_label):
        if icd_list[i] != "nan":
            cluster_dic[c].append(icd_list[i])
    # cluster_dic = sorted(cluster_dic)
    total_idf = []
    for key in range(len(cluster_dic.items())):

        value = cluster_dic[key+1]
        value = ' '.join(value)
        total_idf.append(value)
    X  = vectorizer.fit_transform(total_idf)  
    ...
    for i in range(len(tfidf)):
        ...
        for j in range(len(word)):
            ...
            temp.append([word[j],tfidf[i][j]])

        temp =  sorted(temp,key=lambda x:(x[1],x[0]),reverse = True) 
        # result.append(temp[:10])
        result.append([w[0] for w in temp[:30]])

    print(result)
    #     total_idf.append(word_weight)
        # print(f"cluster {key+1}: ",[w for w in word_weight[:20]])
...
if __name__ == '__main__':
    ...
    test_dataset = PatientDataset(f'/home/comp/cssniu/mllt/dataset/new_packed_data/{visit}/',class_3,visit,flag="train")
