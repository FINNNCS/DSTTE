...
visit = 'twice'
save_dir= "weights"
save_name = f"mllt_transition_{str(loss_ratio)}_label_att_{visit}_0109"
...
def fit(epoch,model,label_token,text_recon_loss,y_bce_loss,data_length,dataloader,optimizer,flag='train'):
    ...
    print("PHASE：{} EPOCH : {} | Micro Precision : {} | Macro Precision : {} | Micro Recall : {} | Macro Recall : {} | Micro F1 : {} |  Macro F1 : {} |  Micro ROC : {} | Macro ROC ： {} | Total LOSS  : {} | Total Cls LOSS  : {} | Total KL LOSS  : {} ".format(flag,epoch + 1, precision_micro,precision_macro,recall_micro,recall_macro, f1_micro,f1_macro,roc_micro,roc_macro, total_loss,total_cls,total_kls))
    if flag == 'test':
        if SV_WEIGHTS:
            if f1_micro > Best_F1:
                Best_F1 = f1_micro
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(best_model_wts, PATH)
    return model,f1_micro, roc_micro       
