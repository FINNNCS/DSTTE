...
from base_model_two_opt import mllt,classifier
# from base_model_bert import mllt
...
visit = 'twice'
save_dir= "weights"
save_name = f"mllt_base_bart_{visit}_1222"
if Logging:
...
start_epoch = 0
weight_dir = "weights/mllt_base_bart_once_1222_epoch_14_loss_0.3307_f1_0.6162_acc_0.7598.pth"
...
def padding_event(vec):
    ...
    for e in vec:
        ...
        word_padding_ids = torch.ones((word_difference,max_subword_length), dtype = torch.long ).to(f"cuda:{input_ids.get_device()}")
...
def fit(epoch,model_vae,model_cls,text_recon_loss,y_bce_loss,data_length,dataloader,optimizer_encoder,optimizer_cls,flag='train'):
    ...
    for i,(cheif_complaint_list,text_list,label_list,event_codes_list,time_stamp_list) in enumerate(tqdm(dataloader)):
        ...
        # RC_loss = torch.zeros(1,len(text_list)).to(device)
...
if __name__ == '__main__':
    train_dataset = PatientDataset(f"dataset/new_packed_data/{visit}/", visit = visit, flag="train")
    ...
    test_dataset = PatientDataset(f"dataset/new_packed_data/{visit}/",visit = visit, flag="test")
    ...
    if pretrained:
        ...
        model_encoder.load_state_dict(torch.load(weight_dir,map_location=torch.device(device2)), strict=True)
        model_cls.load_state_dict(torch.load(weight_dir,map_location=torch.device(device2)), strict=True)

    # model, optimizer = amp.initialize(model, optimizer, opt_level="O1") # 这里是“欧一”，不是“零一”

    ### freeze parameters ####
    optimizer_encoder = optim.Adam(model_encoder.parameters(True), lr = 1e-5)
    optimizer_cls = optim.SGD(model_cls.parameters(), lr = 1e-5, momentum=0.9)
    ...
    for epoch in range(start_epoch,num_epochs):
        ...
        model = fit(epoch,model,text_recon_loss,y_bce_loss,train_length,trainloader,optimizer_encoder,optimizer_cls,flag='train')
        model = fit(epoch,model,text_recon_loss,y_bce_loss,test_length,testloader,optimizer_encoder,optimizer_cls,flag='test')
