    ...
    for i,(cheif_complaint_list,text_list,label_list,event_codes_list,time_stamp_list) in enumerate(tqdm(dataloader)):
        ...
        if flag == "train":
            with torch.set_grad_enabled(True):
                ...
                Yt = \
                model(event_list,text,label_token,self_att)

                # r_loss = label_loss(L_E,regularization_loss, label)
            ...
            with torch.no_grad():
                ...
                Yt = \
                model(event_list,text,label_token,self_att)
                # r_loss = label_loss(L_E,regularization_loss,model, label)
            
                # c_loss = y_bce_loss(Yt.squeeze(),label.squeeze())
                # loss = r_loss + c_loss
...
if __name__ == '__main__':
    ...
    if evaluation:
        ...
        for epoch in range(5):
            ...
            model,roc_micro,roc_macro  = fit(epoch,model,label_embedding,regularization_loss,y_bce_loss,test_length,testloader,optimizer,flag='test')
        ...
        for epoch in range(start_epoch,num_epochs):
            ...
            model,roc_micro,roc_macro = fit(epoch,model,label_embedding,regularization_loss,y_bce_loss,train_length,trainloader,optimizer,flag='train')
            model,roc_micro,roc_macro = fit(epoch,model,label_embedding,regularization_loss,y_bce_loss,test_length,testloader,optimizer,flag='test')
