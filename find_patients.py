...
class PatientDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        for i,v in enumerate(visit_list):
            ...
            if i > 0:
                # print(np.array(label),np.array(last_disease))
                array_div = np.array(label) - np.array(last_disease)
                for a in range(len(array_div)):
                    if array_div[a] == 0:
                        if label[a] != 0:
                            sampe_source = True
                            # print("last label: ", last_disease)
                            # print("label: ", label)
                            # print("....")
            if sampe_source:
                same_number +=1
            if same_number >= 3:
                print(self.text_dir+v)
...
if __name__ == '__main__':
    ...
    visit = 'twice'
    dataset = PatientDataset(f'/home/comp/cssniu/mllt/dataset/new_packed_data/{visit}/',class_3 = True,visit = 'twice',flag="test")
    batch_size = 1
