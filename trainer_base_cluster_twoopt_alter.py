...
visit = 'once'
save_dir= "weights"
save_name = f"basemodel_cluster_10_pretrained_3_weighted_ct_{self_att}_{visit}_0124"
if Logging:
...
# weight_dir = "weights/basemodel_cluster_10_pretrained_3_weighted_ct_sharefc_cross_att_once_0124_epoch_0_loss_0.6938_f1_0.5107_acc_0.5747.pth"
...
def fit(epoch,model_encoder,model_cluster,center_embedding,label_embedding,cluster_loss,y_cluster_bce_loss,test_length,dataloader,optimizer_encoder,optimizer_cluster,flag='train'):
    ...
    l = 0
    if flag == 'train':
        for (i,child) in enumerate(model_cluster.children()):
            if i == 0:
...
if __name__ == '__main__':
    ...
    center_embedding = torch.load("dataset/medical_note_embedding_kmeans_10.pth").type(torch.cuda.FloatTensor)
    ...
    optimizer_cluster = optim.Adam(model_cluster.parameters(True), lr = 1e-4)
    if pretrained:
    ...
    if evaluation:
        ...
        for epoch in range(start_epoch,num_epochs):
            ...
            model_encoder,model_cluster,roc_micro,roc_macro  = fit(epoch,model_encoder,model_cluster,center_embedding,label_embedding,cluster_loss,y_cluster_bce_loss,train_length,trainloader,optimizer_encoder,optimizer_cluster,flag='train1')
