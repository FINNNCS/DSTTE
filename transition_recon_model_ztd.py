...
class mllt(nn.Module):
    ...
    def cross_attention(self,v,c):
        ...
        m = self.drop_out(F.max_pool2d(g,kernel_size = (1,g.shape[-1]))).squeeze(1)  # [b, l, 1]
        b = torch.softmax(m, dim=1)  # [b, l, 1]
        print(b.squeeze().squeeze())
        return b   
    ...
    def approximation(self,Ztd_last,At, Ot,flag):
          
        At_E_batch = self.encoder(**At)[0][:,[0],:].transpose(1,0)
        Ot_E_batch = self.encoder(**Ot)[0]
        Ot_E_batch_cross_attention = self.drop_out(Ot_E_batch * self.cross_attention(Ot_E_batch,At_E_batch)).sum(1)
        # print(self.cross_attention(Ot_E_batch,At_E_batch).squeeze())
        if self.visit == "once":
    ...
    def emission(self,Ztd,At,Ot,Ot_E_batch):
        ...
        index = ( Ot['input_ids'].squeeze(0) == 2).nonzero(as_tuple=True)[0].data
        Ot['input_ids'] = torch.cat((Ot['input_ids'][:,:index],torch.tensor([1],dtype = torch.long).to(Ot['input_ids'].device).unsqueeze(0),Ot['input_ids'][:,index+1:]),-1)
        Ot_mask =  torch.cat((Ot['attention_mask'][:,:index],torch.tensor([0],dtype = torch.long).to(Ot['input_ids'].device).unsqueeze(0),Ot['attention_mask'][:,index+1:]),-1)
        decoder_input_ids = torch.cat((torch.tensor([self.bart.config.decoder_start_token_id],dtype = torch.long).to(Ot['input_ids'].device).unsqueeze(0),\
           Ot['input_ids'][:,1:],At_['input_ids']),-1)
    ...
    def forward(self,Ts,Ct,At,Ot,Ztd_last,Add_additions,flag):
        # print(Ztd_last.shape)

        Ztd,Ztd_mean_post,Ztd_logvar_post,Ot_E_batch = self.approximation(Ztd_last,At, Ot,flag)
